...
#include "../niutensor/tensor/core/movement/CopyValues.h"
using namespace nts;
...
/* constructor */
TripleSample::TripleSample(XTensor a, int myKey) {
    index = -1;
    audioSeq = CopyValues(a);
    // audioSeq = a;
    fLen = a.dimSize[0];
    bucketKey = myKey;
}
...
/* constructor */
TripleSample::TripleSample(string p, int f, int myKey) {
    index = -1;
    audioPath = p;
    fLen = f;
    bucketKey = myKey;
