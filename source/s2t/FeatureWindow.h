...
#include "../niutensor/tensor/core/arithmetic/Mask.h"
...
*/
    ...
    */
    ...
    void OfflineFeatureTpl<F>::ComputeFeatures(
        ...
        audio_length = NumFrames(paddedWave.GetDim(0), computer_.GetFrameOptions());
    ...
    void OfflineFeatureTpl<F>::Compute(
        ...
        *output = SelectRange(*output, 0, 0, paddedRows - 1);
        ...
        if (paddedRows > rows_out) {
            int tempDimSize[] = { 1, cols_out };
            XTensor tempZero; 
            tempZero.Resize(2, tempDimSize, X_FLOAT, 1.0);
            for (int recoverZero = rows_out; recoverZero < paddedRows - 1; recoverZero++) {
                output->SetData(tempZero.GetCell(startIndex, 2), cols_out, recoverZero * cols_out);
            }
