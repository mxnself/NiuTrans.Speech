...
{   
    ...
    */
    ...
    {   
        ...
        if(model.NAR){
            predictor.Load(argsNum, (const char**)args);
        }
    ...
    */
    ...
    void S2TConfig::showConfig()
    {
        ...
        cout << "\t| " << "idim = " << predictor.idim << endl;
        cout << "\t| " << "adapterLayerNum = " << model.adapterLayerNum << endl;
        ...
        cout << "\t| " << "threshold = " << predictor.threshold << endl;
        cout << "\t| " << "tail_threshold = " << predictor.tail_threshold << endl;
    }
    ...
    void PredictorConfig::Load(int argsNum, const char** args)
    {
        Create(argsNum, args);
        LoadInt("idim", &idim, 0);
        LoadFloat("threshold", &threshold, 0);
        LoadFloat("tail_threshold", &tail_threshold, 0);
    }

    /* load whisper configuration from the command */
    void WhisperDecConig::Load(int argsNum, const char** args) {
        Create(argsNum, args);
        LoadInt("numLanguage", &numLanguage, 99);
