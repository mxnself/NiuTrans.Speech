 ...
 * $Modified by: GE Yuan (geyuanqaq@gmail.com) 2024-09 (Non-Autoregressive)
 */
...
void AttDecoder::InitModel(NMTConfig& config)
{
    ...
    for (int i = 0; i < nlayer; i++) {
        ...
        selfAtts[i].InitModel(config, false, true, config.model.fusionAttn);
        enDeAtts[i].InitModel(config, false, false, config.model.fusionAttn);
                         ...
                         XTensor* mask, XTensor* maskEncDec, int nstep)
{
    ...
    for (int i = 0; i < nlayer; i++) {
        ...
        ende = enDeAtts[i].Make(outputEnc, endeAttnBefore, outputEnc, maskEncDec, &enDeAttCache[i]);
...
XTensor AttDecoder::RunFastPreNorm(XTensor& inputDec, XTensor& outputEnc, XTensor* maskEncDec, int nstep)
{
    ...
    for (int i = 0; i < nlayer; i++) {
        ...
        /* self attention */
        xn = selfAtts[i].Make(xn, xn, xn, NULL, &selfAttCache[i]);
...
XTensor AttDecoder::RunFastPreNorm(XTensor& inputDec, XTensor& outputEnc, XTensor* mask, XTensor* maskEncDec, int nstep)
{
    ...
    for (int i = 0; i < nlayer; i++) {
        ...
        /* self attention */
        xn = selfAtts[i].Make(xn, xn, xn, mask, &selfAttCache[i]);
        ...
        /* encoder-decoder attention */
        x = enDeAtts[i].Make(outputEnc, x, outputEnc, maskEncDec, &enDeAttCache[i]);
...
XTensor AttDecoder::RunFastPostNorm(XTensor& inputDec, XTensor& outputEnc, XTensor* maskEncDec, int nstep)
{
    ...
    for (int i = 0; i < nlayer; i++) {
        ...
        /******************/
        /* self attention */
        xn = selfAtts[i].Make(x, x, x, NULL, &selfAttCache[i]);
...
>> outputEnc - the output tensor of the encoder
>> maskEncDec - mask for the encoder-decoder attention
>> outputPre - the output tensor of the CifPredictor
>> maskPreDec - mask for the predictor-decoder attention
<< return - the output tensor of the decoder
*/
XTensor AttDecoder::RunFastNarPreNorm(XTensor& outputEnc, XTensor* maskEncDec, XTensor& outputPre, XTensor* maskPreDec)
{
    /* clear the history */
    if (useHistory)
        history->ClearHistory();
    ...
    // x.Dump(stderr, "outxputPre", 5);
    // cout << "------ x in decoder: " << x.GetDim(0) << x.GetDim(1) << x.GetDim(2)  <<"------"<< endl;

    if (useHistory)
        history->Add(outputPre);
    ...
    for (int i = 0; i < nlayer; i++) {
        ...
        xn = selfAtts[i].Make(xn, xn, xn, maskPreDec, &selfAttCache[i]);
